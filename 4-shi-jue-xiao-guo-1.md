...
当图片需要显示不同的大小的时候，有一种叫做_拉伸过滤_的算法就起到作用了。它作用于原图的像素上并根据需要生成新的像素显示在屏幕上。
